...
export class MemberMessages implements OnInit, OnDestroy {
  ...
  constructor() {
    ...
    // Debounce typing to avoid spamming
    this.typingSubject.pipe(debounceTime(1000)).subscribe(() => {
      if (this.recipientId) {
        this.presenceService.sendTypingNotification(this.recipientId);
        this.scrollToBottom();
      }
    });
  }
  ...
  getReceipientName() {
    return this.memberService.member()?.displayName;
  }
}
